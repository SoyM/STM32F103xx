...
void SPI2_Init(void)	
{
	...
	GPIOB->ODR|=1<<14;
	GPIOB->ODR|=1<<15;
	SPI2->CR1|=0<<15;//设置SPI单向或者双向的数据模式:SPI设置为双线双向全双工
	SPI2->CR1|=0<<10;
	SPI2->CR1|=1<<2;//设置SPI工作模式:设置为主SPI
	SPI2->CR1|=0<<6;
	SPI2->CR1|=0<<11;//设置SPI的数据大小:SPI发送接收8位帧结构
	SPI2->CR1|=0<<1;//串行同步时钟的空闲状态为低电平
	SPI2->CR1|=0<<0;	//串行同步时钟的第一个跳变沿（上升或下降）数据被采样
	SPI2->CR1|=1<<9;//NSS信号由硬件（NSS管脚）还是软件（使用SSI位）管理:内部NSS信号有SSI位控制
	SPI2->CR1|=7<<3;//定义波特率预分频的值:波特率预分频值为256
	SPI2->CR1|=0<<7;	//指定数据传输从MSB位还是LSB位开始:数据传输从MSB位开始
	SPI2->CRCPR|=7;	//CRC值计算的多项式
	SPI2->CR1|=1<<6;   //使能SPI外设
